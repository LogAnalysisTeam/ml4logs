...
persistence = FilePersistence(output_dir + "/Thunderbird_Drain3.state.bin")
...
structure_id = []
lines = []
with open(input_log, encoding="latin1") as f:
    for line in f:
        line = line.rstrip()
...
pd.DataFrame(templates_out, columns=["EventId", "Occurrences", "EventTemplate"]). \
    to_csv(output_dir + "/Thunderbird_Drain3.log_templates.csv", index=False)
...
pd.DataFrame(structured_out, columns=["EventId", "Content", "EventTemplate"]). \
    to_csv(output_dir + "/Thunderbird_Drain3.log_structured.csv", index=False)
