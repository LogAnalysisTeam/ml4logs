...
from data import dataloader_BGL
...
if __name__ == '__main__':
    (x_tr, y_train), (x_te, y_test) = dataloader_BGL.load_BGL(pjoin(log_path, log_file),
                                                          save_path=log_path,
                                                          window='sliding',
                                                          time_interval=60,
                                                          stepping_size=60,
    ...
    for _model in run_models:
        ...
        if _model == 'PCA':
            ...
            train_dataset = feature_extractor.fit_transform(*dataloader_BGL.slice_hdfs(x_tr, y_train, window_size))
            test_dataset = feature_extractor.transform(*dataloader_BGL.slice_hdfs(x_te, y_test, window_size))
